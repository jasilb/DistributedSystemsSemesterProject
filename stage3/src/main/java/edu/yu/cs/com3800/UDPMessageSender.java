...
public class UDPMessageSender extends Thread implements LoggingServer {
    ...
    public UDPMessageSender(LinkedBlockingQueue<Message> outgoingMessages, int serverUdpPort) {
        ...
        setName("edu.yu.cs.com3800.UDPMessageSender-port-" + this.serverUdpPort);
    }
    ...
    public void run() {
        while (!this.isInterrupted()) {
            try {
                ...
                if (messageToSend != null) {
                    ...
                    socket.close();
                    this.logger.fine("edu.yu.cs.com3800.Message sent:\n" + messageToSend.toString());
                }
            }
            catch (IOException e) {
        ...
        this.logger.log(Level.SEVERE,"Exiting edu.yu.cs.com3800.UDPMessageSender.run()");
    }
}